...
| `tools/ste-check.mjs` | The prose checker reads comments and Markdown, strips the code out, and reports each sentence that breaks a rule. It checks the Simplified Technical English mechanics everywhere. It checks the register rules in source comments alone, because a document states requirements and a comment describes code. A rule a machine cannot judge with confidence prints as an advisory note, which carries no exit code. It runs before every commit. |
...
| `formula/format.ts` | This file turns an AST back into source text. It maps IDs back to current names. This is what lets the properties panel and the `props` command show a formula the way the operator wrote it. |
...
11. **`.text-editor` sets no font, no padding and no border.** It sets
...
4. **`addvertex`, `delvertex` and `explode` all exist now.** `addvertex`
   ...
   formula that reads one of those needs no repair. A formula that reads a
