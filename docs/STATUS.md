...
| Tests | 2109 pass, 0 skip, across 39 test files. |
...
5. **Arcs and `split` exist now. Beziers do not.** A `vertex.N.bulge` slot
   ...
   lands on an end, where a vertex already sits. One thing stays open here. A
   circle can now drop its `vertices` slot, because two vertices and two bulges
   of 1 describe a circle exactly, and the 32 point approximation then earns no
   place. Its bounds, hit test and `explode` each go exact in the same change.
