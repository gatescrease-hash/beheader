...
/** A `text` object with a drawn extent — `resolvedContent` set, `width` fixed and `measuredHeight` present so the box is (originX,originY)-(originX+w, originY+h). */
function textObject(originX: number, originY: number, width: number, height: number): GraphObject {
  return {
    id: "obj_text",
    name: "text_1",
    type: "text",
