 ...
 *     **This claim was FALSE from entry 0089 until the load-hardening cycle
 *     (D-108 clause 1, re-owned by D-127).** `reconstructSlot` cast `raw.ast as
 *     FormulaAst` unchecked, so a loaded `ast` of `null`, a `binaryOp` with absent
 *     or `null` children, or a `functionCall` whose `args` was not an array threw a
 *     `TypeError` out of `loadDocument` — and through `main.ts`'s `openDocument`
 *     that became an unhandled promise rejection, i.e. the operator saw NOTHING
 *     happen. `formula/ast.ts`'s `validateFormulaAstShape` now closes it at the
 *     boundary, once, and the four shapes are pinned in `document.test.ts`. The
 *     history is kept here rather than deleted because a header that has been wrong
 *     once is worth knowing about: this is what stops the next reader assuming.
 ...
 *     its motivating case.
 *   - **Which derived slots exist comes from the SCHEMA, not from the file**
 *     (**D-126**, `withSchemaDerivedSlots`). The file carries a derived slot's key
 *     but never its value, and taking that key set as authoritative meant adding one
 *     derived slot to a schema invalidated every document saved before it. §5.11
 *     already says derived state is regenerated on load; this makes that true of the
 *     slot set too.
 ...
 * validation. Never throws — genuinely, as of the load-hardening cycle; the file
 * header records the period during which that claim was false (D-108/D-127).
 *
...
/**
 * **D-126**: the SCHEMA says which derived slots an object has — never the file.
 *
 * The defect this closes: `serializeSlot` drops a derived slot's VALUE (§5.11) but
 * keeps its KEY, and the old reconstruction took the file's key set as
 * authoritative. D-018 then requires every schema-declared derived path to be
 * present, so **adding one derived slot invalidated every previously saved document
 * containing that object type** — a document saved before entry 0141 was refused
 * with `text_1.measuredWidth is missing`, which reads to the operator as a corrupt
 * file rather than as a build that moved on. §5.11 already says derived state is
 * "regenerated by a full evaluation pass on load"; this makes that true of the slot
 * SET as well as the values.
 *
 * Two directions, both from clause 2:
 *   - every declared derived path with no slot in the file gets the same
 *     `{ kind: "derived", value: null }` placeholder `createObjectFromCommand`
 *     writes at creation — the same mechanical fill from the same source of truth;
 *   - a `derived`-kind slot in the file whose path the schema does NOT declare is
 *     DROPPED. §5.1: "`derived` is fixed by schema", so a stale one from an older
 *     build is not state to preserve.
 *
 * **What it deliberately does NOT do (clause 3): it never overwrites a
 * non-`derived` slot.** A file holding a `literal` at a declared derived path is
 * still D-018 case 2's rejection — that check is not weakened and does not move.
 * This removes exactly one way to reach it (schema drift across a save) and none of
 * the others.
 *
...
function withSchemaDerivedSlots(type: ObjectType, slots: Record<string, Slot>): Record<string, Slot> {
  ...
  for (const key of Object.keys(slots)) {
    ...
    if (slot.kind === "derived" && !declared.has(key)) {
      continue; // A derived slot this build's schema no longer declares.
    }
    rebuilt[key] = slot;
  }
...
function reconstructSlot(raw: unknown, objectName: string, key: string): SlotReconstructionResult {
  ...
  if (raw.kind === "formula") {
    ...
    // **D-108 clause 2 / D-127**: the ONE shape validation a loaded AST passes
    // through. This used to be `raw.ast as FormulaAst`, an unchecked cast — so a
    // loaded `ast` of `null`, a `binaryOp` with `null` or absent children, or a
    // `functionCall` whose `args` was not an array threw a `TypeError` straight
    // out of `loadDocument`, which `main.ts`'s `openDocument` turned into an
    // unhandled promise rejection: no refusal, no log line, nothing happened.
    // It runs BEFORE the depth check because the depth check itself walks the
    // AST — see `validateFormulaAstShape` for why that ordering is safe.
