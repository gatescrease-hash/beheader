...
function objects(...pairs: Array<[id: string, name: string]>): AddressableObject[] {
...
describe("parseAddress", () => {
  it("resolves a simple table-cell address to {objectId, path}", () => {
...
describe("formatAddress", () => {
  it("round-trips a parsed address back to its canonical string", () => {
    const docObjects = objects(["obj_3", "table_x"]);
    const parsed = parseAddress("table_x.A1", docObjects);
    expect(formatAddress(parsed as never, docObjects)).toBe("table_x.A1");
  });
  ...
  it("reflects a rename with no change to the stored address", () => {
    ...
    const stored = parseAddress("polygon_1.origin.x", before);
    ...
    expect(Object.keys(stored as object).sort()).toEqual(["objectId", "path"]);
    ...
    expect(formatAddress(stored as never, after)).toBe("intersection_a.origin.x");
    expect(formatAddress(stored as never, before)).toBe("polygon_1.origin.x");
  });
});
