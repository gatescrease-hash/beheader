...
## D-126 — Adding a `derived` slot to a schema is a LOAD-COMPATIBILITY event: the loader reconstructs a schema's declared derived slots, and never trusts a saved file to list them
Answers: finding F23 at 0142-REVIEW-phase5 (probed)   Ruled: entry 0142-REVIEW-phase5 (reviewer)
Binding on: `src/engine/document.ts` (`reconstructObject` / the slot-rebuild path), and every
future cycle that adds a `derived` slot to any schema in `primitives/schema.ts`

**Ruling.**

1. **State the defect plainly.** `serializeSlot` writes a `derived` slot as `{ kind: "derived" }` —
   no value, per §5.11. `reconstructSlot` turns that back into `{ kind: "derived", value: null }`.
   So the file does not carry the derived VALUE, but it does carry the derived slot's KEY, and
   the reconstruction takes the file's key set as authoritative. **D-018** then requires every
   schema-declared derived path to be present. The consequence, probed at this review: a document
   saved before entry 0141 with a `text` object is REFUSED on load with
   `text_1.measuredWidth is missing — deriveEdges still emits an edge into it, pointing at a slot
   that does not exist (D-018)`. Adding one derived slot invalidated every previously saved
   document containing that object type.

2. **The loader MUST reconstruct the schema's declared derived slots rather than read them from
   the file.** For an object whose `type` has a schema, the derived slot set comes from
   `getObjectSchema(type).derivedSlots`, each rebuilt as `{ kind: "derived", value: null }` — the
   same mechanical fill `createObjectFromCommand` already performs on creation, and the same source
   of truth. A `derived` key in the file that the schema does not declare is dropped, not carried:
   the schema is what says which derived slots exist (§5.1, "`derived` is fixed by schema").

3. **D-018's check is NOT weakened and does not move.** It still runs in `mutation.ts` step 4 over
   the reconstructed object, and it still refuses an object whose derived slot set disagrees with
   its schema. Clause 2 removes one way to reach that refusal (schema drift across a save); it
   removes none of the others — an object whose type has no schema, a hand-built object, a
   `literal` written over a derived path (D-018 case 2). The rejection this ruling eliminates is the
   one that punishes the operator for a change made after they saved.

4. **`formatVersion` is NOT bumped for a derived-slot addition, and the alternative is rejected on
   the record.** Bumping `FORMAT_VERSION` would make `deserializeDocument` refuse the old file
   outright — a worse outcome for the same cause, and it would make every schema extension a
   breaking format change. §5.11 already says derived values "are regenerated by a full evaluation
   pass on load"; clause 2 is what makes that sentence true of the slot set as well as the values.

5. **Every future cycle that adds a derived slot states the load consequence in its log entry** —
   one line, naming the object types whose saved documents are affected. Once clause 2 is built the
   honest line is "none"; until it is built, the honest line names them. Entry 0141 owed this line
   and did not write it, which is the finding — not the schema change, which is D-123 correctly
   executed.

**Rationale.** **D-120**'s own rationale asserts the opposite of the observed behaviour — "a
`derived` slot whose value never serializes, so no stored document can depend on the answer" — and
**D-123**'s reversibility argument rests on the same reading. The reading is half right: the value
does not serialize, the key does, and D-018 makes the key load-bearing. That inference has now been
made twice in reviewer rulings and acted on once in an implementer cycle without anyone probing it,
which is the signature of a shared wrong model rather than a slip. §5.11's own words settle the
direction: derived state is regenerated on load, so the file is not the authority on it.

This is reachable today and not theoretical: §5.11's `save` and `load` commands are both built and
operator-reachable (`main.ts`'s `downloadDocument` / `openDocument`), and `text` objects have been
creatable since entry 0136. The cost of the fix is small and local to `document.ts`. The cost of not
fixing it grows with every schema extension — markdown-lite and `overflow` are next, and Phase 6's
`script.out.*` adds derived slots per object.

**Reversible; the human may overrule.** If a saved file's derived key set is ever wanted as a CHECK
on schema drift rather than as data, the answer is a named diagnostic at the load boundary, not a
refusal that reads to the operator as a corrupt file.

Reconciliation required: owed by the load-hardening cycle named in **D-127** — the same cycle, the
same file, one diff. It does NOT displace **D-125** / **D-124**, which the human ruled ABSOLUTE
PRIORITY. `document.test.ts` gains the round-trip this ruling implies: a document serialized under a
schema with N derived slots loads under a schema with N+1, and the new slot arrives as its
placeholder.

---

## D-127 — D-108's deferral condition can never fire: §5.11's file input IS built, the malformed-AST throw IS operator-reachable, and the loader hardening now has a named owner
Answers: finding F24 at 0142-REVIEW-phase5 (probed)   Ruled: entry 0142-REVIEW-phase5 (reviewer)
Binding on: `src/engine/document.ts`, `src/main.ts`'s `openDocument`; **amends D-108 clause 3**

**Ruling.**

1. **D-108's stated premise is factually false, and was false when it was written.** Its clause 3
   rationale says "It is not reachable by any operator today — §5.11's file input is unbuilt, so
   `loadDocument` has no caller outside tests." `main.ts`'s `openDocument` — a real
   `<input type="file">` whose `change` handler calls `loadDocument` — landed at **entry 0089**,
   twenty-four entries BEFORE D-108 was ruled at 0113-REVIEW. `save` and `load` are both live
   registry entries in `parser.ts`. No cycle has ever been able to satisfy "the cycle that builds
   §5.11's file input," because that cycle is in the past.

2. **The defect is worse than D-108 documented.** Probed at this review: all four shapes D-108
   clause 1 names — `ast: null`, a `binaryOp` with `null` children, a `binaryOp` with absent
   children, a `functionCall` whose `args` is not an array — throw a `TypeError` out of
   `loadDocument`. Because `openDocument` calls it inside `file.text().then(...)`, the throw becomes
   an **unhandled promise rejection**: the operator gets no refusal message, no log line, and no
   indication the load failed — the program simply does nothing. D-108 clause 1 describes a broken
   "never throws" claim; the operator-facing behaviour is a silent no-op, which is what §5.10's
   "every rejection message must name the specific slots involved" exists to prevent.

3. **D-108 clauses 1, 2 and 4 stand unchanged.** One shape validation at the load boundary, against
   `FormulaAst`'s own variants; no walker hardened piecemeal; `parser.ts`'s `default:` posture
   untouched. This ruling changes the OWNER, not the shape of the fix. Clause 3's prohibition on
   piecemeal hardening survives with it, and is not weakened by the wait now having an end.

4. **The owner is a named load-hardening cycle**, which also discharges **D-126** — same file, same
   boundary, one diff. It runs AFTER **D-125** (in-place text entry) and **D-124** (`text` placed by
   pointing), which the human ruled ABSOLUTE PRIORITY at entry 0140; nothing here reorders that. It
   MUST report `REVIEW: REQUIRED` — `document.ts` is §6.2 load-bearing.

5. **That cycle's visible diff, so it is checkable:** the boundary validation of clause 2;
   `document.test.ts` extended to the four shapes of D-108 clause 1 (that ruling's own
   reconciliation note, still owed); D-126's schema-reconstruction and its round-trip test; and
   `openDocument`'s promise chain given a rejection path, so a throw that escapes anything reaches
   `onRefused` rather than the console. Correcting the two "never throws" doc claims is part of it
   (D-108 clause 1), not a separate errand.

**Rationale.** A binding ruling that defers work to a trigger which cannot fire is worse than an
open fix-list item: it reads as owned, `STATUS.md` has carried "D-108 IS OWED BY §5.11's LOAD CYCLE"
forward through every rewrite since, and clause 3 actively forbids anyone else from touching it. The
error was a reviewer's, not an implementer's — 0113-REVIEW reasoned about the reachability of a code
